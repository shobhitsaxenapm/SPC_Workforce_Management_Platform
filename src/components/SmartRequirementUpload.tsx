...
import { ExtractedRequirementData } from '../types';
...
export default function SmartRequirementUpload({ onExtractionSuccess, onCancel }: SmartRequirementUploadProps) {
  ...
  const validateAndSetFile = (f: File) => {
    ...
    if (!validTypes.includes(f.type)) {
      setError('Unsupported file format. Please upload PDF, DOCX, XLSX, or TXT.');
      return;
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] w-full max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-sm">
      ...
      {!file ? (
        ...
        >
          ...
          <p className="text-sm text-slate-500 mt-2">Supports PDF, DOCX, XLSX, TXT (Max 10MB)</p>
          <input 
            type="file" 
            ref={fileInputRef} 
            className="hidden" 
            accept=".pdf,.docx,.doc,.xlsx,.xls,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/plain"
          ...
          {status === 'Processing' || status === 'Uploading' ? (
            <div className="flex flex-col items-center justify-center py-6 space-y-4">
              ...
              <div className="text-center">
                <p className="font-medium text-slate-800">{status === 'Uploading' ? 'Uploading document...' : 'AI is extracting requirement details...'}</p>
                <p className="text-sm text-slate-500">This may take a few moments. If the document contains multiple roles, they will be extracted automatically.</p>
              </div>
            </div>
          ) : (
